...
def removeNans(baselist, xlists = None, keeplongest = True):
    """
    Splits the given list into sequences of NaN and non-NaN values and returns
    a list with only the sequences of non-NaN values
    
    Inputs
    ------
    
    baselist, list of lists: data for which to extract sequences of NaN values
    
    xlists, list of lists : corresponding x data, if set to None, only the 
        baselist is treated
        
    keeplongest, bool : if set to True, only the longest sequence of non-Nan 
        values are kept.
        
    Returns
    -------
    
    corr_list, list of lists : the list with only sequences of non-NaN values

    corr_xlist, list of lists : corresponding x values
    ...
    if xlists is None:
        ...
        if keeplongest is True:
            for i in range(0, len(baselist)):
                l = []
                vv = [[key, list(group)] for key, group in groupby(baselist[i], key=np.isnan)]
                for j in range(0, len(vv)):
                    if vv[j][0] == False:
                        l.append( vv[j][1] )
                try:
                    att = max( [[len(subl), subl] for subl in l if isinstance(subl, list)] )[1]
                except ValueError:
                    raise ValueError('could not find non-Nan list')
                    #print('could not find non-Nan list')
                corr_list.append( att )
        else:
        ...
        if keeplongest is True:
            for i in range(0, len(baselist)):
                ...
                for j in range(0, len(vv)):
                    ...
                    k+=len(vv[j][1])
                try:
                    lens, att, att2 = max( [[len(subl), subl, subx] for (subl, subx) in zip(l,x) if isinstance(subl, list)] )
  
                except ValueError:
                    raise ValueError('could not find non-Nan list')
                    #print('could not find non-Nan list')
                corr_list.append( att )
                corr_xlist.append( att2 )
        else:
            ...
            for i in range(0, len(baselist)):
                ...
                for j in range(0, len(vv)):
                    ...
                    if vv[j][0] == False:
                        corr_list.append( vv[j][1])
                        corr_xlist.append( xlist[k:t] )
                    k+=len(vv[j][1])
                    
        return corr_list, corr_xlist
...
def prepareData(mldata, xdata, lim = 80):
    """
    ...
    ml = ml[0]
    x = x[0]
    medml = ndimage.median_filter(ml,50)
    if len(ml) > lim :
